...
function missingSchemaMessage(error: unknown) {
  ...
  if (message.toLowerCase().includes('bucket not found')) {
    return `Recording bucket "${recordingBucket}" was not found. Run supabase/rtc_calls_setup.sql in the main Supabase project.`;
  }
  if (message.includes('recording_path') || message.toLowerCase().includes('bucket') || message.toLowerCase().includes('storage')) {
    return `Call recording storage is not ready. Run supabase/rtc_calls_setup.sql in the main Supabase project to create the "${recordingBucket}" bucket.`;
  }
  return null;
}
