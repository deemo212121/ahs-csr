...
// Shared by both /customer/register and /customer/register/[companyId] — a
// companySlug hides the manual company dropdown entirely (tenancy is already
// decided by the link the customer arrived on) and gets sent to the API,
// which re-resolves it against Firestore server-side rather than trusting a
// client-supplied company_id.
export function RegisterPageContent({ companySlug }: { companySlug?: string }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/companies')
      ...
      .catch(() => {
        // Non-fatal — the dropdown just stays empty and submit will surface
        // a clear error if a company is actually required.
      });
    return () => {
      cancelled = true;
    };
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (!companySlug && companies.length && !companyId) throw new Error('Please select a company.');

      const fullName = `${firstName.trim()} ${lastName.trim()}`.trim();
      const { confirmationRequired } = await registerCustomerEmail(email, password, {
        full_name: fullName,
      ...
      if (confirmationRequired) {
        setMessage(`Account created! We sent a confirmation link to ${email}. Please check your inbox before logging in.`);
      } else {
        setMessage('Customer account created. You can now log in.');
        setTimeout(() => router.push(companySlug ? `/login/${companySlug}` : '/login'), 900);
      }
    } catch (err) {
  ...
  const loginHref = companySlug ? `/login/${companySlug}` : '/login';

  if (companyState === 'checking') {
    return (
      ...
      <div className="register-container-php">
        <div className="register-card-php">
          <div className="register-header-php">
            ...
            <p>{companyName ? `Join ${companyName} to get professional appliance repair` : 'Join Home Service to get professional appliance repair'}</p>
          </div>
          ...
          <form className="register-body-php" onSubmit={submit}>
            ...
            <div className="login-link-php">
              Already have an account? <Link href={loginHref}>Login here</Link>
            </div>
          </form>
        </div>
      </div>
